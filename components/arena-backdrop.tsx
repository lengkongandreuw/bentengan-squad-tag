...
export const arenaImage = (id: string) => publicAsset(`arena-ui/${id}.webp?v=1`);
export const arenaVideo = (id: string) => publicAsset(`arena-ui/${id}.mp4?v=1`);
...
export function ArenaBackdrop({ id, video = false, onEnded }: {
  ...
  return <div className="arena-backdrop" aria-hidden="true">
    <img key={id} src={arenaImage(id)} alt="" fetchPriority="high" />
    {video && !reducedMotion && !failed && <video key={id} src={arenaVideo(id)}
