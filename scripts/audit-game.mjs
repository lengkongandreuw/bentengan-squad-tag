...
assert(manifest.version === 5 && manifest.characters.length === 12, 'manifest sprite v5 memuat 12 karakter');
assert(manifest.atlas.columns === 7 && manifest.atlas.width === 1344 && manifest.atlas.height === 1344 && manifest.atlas.rows === 6, 'atlas produksi konsisten 7×6');
...
for (const id of ids) {
  ...
  assert(atlasMeta.width === 1344 && atlasMeta.height === 1344 && atlasMeta.hasAlpha, `${id}: atlas 1344×1344 transparan`);
  assert(animation.version === 5 && animation.source.columns === 7 && animation.atlas.columns === 7, `${id}: metadata grid 7×6 sinkron`);
  assert(animation.quality.frameCount === 42 && animation.quality.minimumRetainedRatio >= .97, `${id}: 42 frame utuh (${(animation.quality.minimumRetainedRatio * 100).toFixed(2)}% piksel penting dipertahankan)`);
  ...
  let gutterLeaks = 0, weakFrames = 0;
  for (let row = 0; row < 6; row++) for (let column = 0; column < 7; column++) {
    const left = column * 192, top = row * 224;
    let frameOpaque = 0;
    for (let y = 4; y < 220; y++) for (let x = 4; x < 188; x++) if (atlas[((top + y) * atlasInfo.width + left + x) * 4 + 3] > 20) frameOpaque++;
...
console.log(`\nAUDIT LULUS · ${ids.length} karakter · 2 tim tetap · ${manifest.characters.length * 42} frame terjaga`);
