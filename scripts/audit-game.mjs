...
assert(fieldManifest.version === 5 && Object.keys(fieldManifest.objects.assets).length === 75, 'manifest field v5 memuat 75 objek statis serta terrain Map 1 dan Map 2');
...
const fieldRuntimeBytes = (await stat(fieldObjectsPath)).size + (await stat(fieldAnimatedPath)).size + (await stat(fieldGroundsPath)).size + (await stat(kampungMapPath)).size + (await stat(pasarMapPath)).size;
const fieldDecodedBytes = (fieldObjects.width ?? 0) * (fieldObjects.height ?? 0) * 4 + (fieldAnimated.width ?? 0) * (fieldAnimated.height ?? 0) * 4 + (fieldGrounds.width ?? 0) * (fieldGrounds.height ?? 0) * 4 + (kampungMap.width ?? 0) * (kampungMap.height ?? 0) * 4 + (pasarMap.width ?? 0) * (pasarMap.height ?? 0) * 4;
assert(fieldObjects.width === 2048 && fieldObjects.height === 2176 && fieldObjects.hasAlpha, 'atlas objek statis v5 2048×2176 transparan dan terpotong rapat');
...
assert(fieldRuntimeBytes <= 2300 * 1024, `lima aset field ${(fieldRuntimeBytes / 1024).toFixed(0)} KiB berada dalam budget 2.300 KiB`);
assert(fieldDecodedBytes <= 38 * 1024 * 1024, `memori decode field ${(fieldDecodedBytes / 1024 / 1024).toFixed(1)} MiB berada dalam budget 38 MiB`);
...
assert(prototypeSource.includes("getFieldImage('objects.webp')") && prototypeSource.includes("getFieldImage('animated.webp')") && prototypeSource.includes("getFieldImage('grounds.webp')") && prototypeSource.includes("background: 'kampung-map.webp'") && prototypeSource.includes("background: 'pasar-map.webp'"), 'dekorasi memakai tiga atlas dan dua map revisi memakai terrain komposit khusus');
...
assert(prototypeSource.includes('const W = 1538') && prototypeSource.includes('const H = 1096') && prototypeSource.includes('const WORLD_SCALE_X = W / DESIGN_W'), 'arena default memakai rasio terrain 1538×1096 sementara Map 1 dan Map 2 memiliki dimensi khusus');
...
assert((prototypeSource.match(/objectScale: (?:0\.9|MAP_OBJECT_SCALE)/g) ?? []).length >= 2 && prototypeSource.includes('(x + w / 2) * scaleX - mapW(w) / 2') && prototypeSource.includes('item.hidden ||') && prototypeSource.includes('item.underlay ||'), 'Map 1 dan Map 2 diperluas 15%, objek diperkecil 10% dari titik tengah, dan collider bantu atau margin tidak digambar ulang');
...
assert(fieldBaseline.version === 5, 'golden baseline field v5 mencakup sumber Map 1 dan Map 2');
