...
assert(prototypeSource.includes('ui-v2/${file}?v=') && prototypeSource.includes("file.startsWith('controls/team-red-') ? 9 : 8") && !prototypeSource.includes('asset-inbox/') && !prototypeSource.includes('Assets/pictures/'), 'runtime memakai UI v8 dan koreksi logo merah v9 tanpa merujuk PNG sumber');
...
assert((prototypeSource.match(/p\.characterId === 'raja' \|\| p\.characterId === 'jago'/g) ?? []).length === 2 && (prototypeSource.match(/\? direction === 'west'/g) ?? []).length >= 2, 'Raja dan Jago memakai frame kanan asli serta mirror hanya ketika menghadap kiri');
...
const pasarMapPath = path.join(root, 'public/field/pasar-map.webp');
const tamanMapPath = path.join(root, 'public/field/taman-map.webp');
const kanalMapPath = path.join(root, 'public/field/kanal-map.webp');
const kanalWaterMaskPath = path.join(root, 'public/field/kanal-water-mask.png');
...
const pasarMap = await sharp(pasarMapPath).metadata();
...
assert(kampungMap.width === 1769 && kampungMap.height === 1260 && !kampungMap.hasAlpha, 'terrain Map 1 memiliki dimensi gameplay 15% lebih luas tanpa kanal alpha');
assert(pasarMap.width === 1923 && pasarMap.height === 1082 && !pasarMap.hasAlpha, 'terrain Map 2 memiliki dimensi gameplay 15% lebih luas tanpa kanal alpha');
...
assert(prototypeSource.includes("getFieldImage('objects.webp')") && prototypeSource.includes("getFieldImage('animated.webp')") && prototypeSource.includes("getFieldImage('grounds.webp')") && prototypeSource.includes("background: 'kampung-map.webp'") && prototypeSource.includes("background: 'pasar-map.webp'") && prototypeSource.includes("background: 'taman-map.webp'") && prototypeSource.includes("background: 'kanal-map.webp'"), 'dekorasi memakai tiga atlas dan keempat map memakai background komposit khusus');
...
assert(prototypeSource.includes('const W = 1538') && prototypeSource.includes('const H = 1096') && prototypeSource.includes('const WORLD_SCALE_X = W / DESIGN_W'), 'arena default memakai rasio terrain 1538×1096 sementara Map 1, Map 2, dan Map 3 memiliki dimensi khusus');
assert(prototypeSource.includes('const STATIC_MAP_SCALE = 0.5') && prototypeSource.includes('field.structuresInBackground') && prototypeSource.includes('staticLayer.width = Math.round(worldWidth * staticMapScale)'), 'cache visual Map 1, Map 2, dan Map 4 diraster setengah resolusi; sheet objek final Map 3 memakai resolusi 75%');
assert((prototypeSource.match(/objectScale: (?:0\.9|MAP_OBJECT_SCALE)/g) ?? []).length >= 2 && prototypeSource.includes('structuresInBackground: true') && prototypeSource.includes('(x + w / 2) * scaleX - mapW(w) / 2') && prototypeSource.includes('item.hidden ||') && prototypeSource.includes('item.underlay ||'), 'Map 1 dan Map 2 mempertahankan skala objek 90%, sedangkan Map 3 memakai sheet objek final dengan collider tersembunyi');
...
assert(prototypeSource.includes('navigateAroundHazards') && prototypeSource.includes('directionIsTraversable') && prototypeSource.includes('blocked(x, y, p, now) || isWaterAt(x, y)') && prototypeSource.includes("if (me.state === 'RETURNING')"), 'AI dan pemain yang pulang dari penjara memilih jalur yang menghindari collider serta sungai');
assert(prototypeSource.includes('findParkourLanding') && prototypeSource.includes('maximumDistance = Math.max(nominalDistance, 132)') && prototypeSource.includes('landing.crossedWater'), 'parkour sungai mencari titik darat aman secara adaptif sebelum memindahkan pemain');
assert(globalStyles.includes('.team-pick-green{inset:50% 0 0 0}') && globalStyles.includes('@media(orientation:portrait) and (max-width:620px)') && globalStyles.includes('transform:rotate(90deg)'), 'pemilihan tim ponsel memiliki dua area aktif dan tampilan portrait otomatis menjadi landscape');
assert(prototypeSource.includes('playerMechanicsLocked') && prototypeSource.includes('MENUNGGU DIBEBASKAN') && prototypeSource.includes('disabled={playerMechanicsLocked}') && globalStyles.includes('.action-dock.mechanics-inactive'), 'mekanik pemain tahanan dinonaktifkan sambil menampilkan peringatan menunggu pembebasan');
assert(prototypeSource.includes('ultimate-roster-badge') && prototypeSource.includes('Ultimate Raja dan Kaka') && prototypeSource.includes('computed-status') && prototypeSource.includes('arena-intel'), 'seleksi karakter menandai pemilik Ultimate dan HUD/rules menjelaskan status terhitung');
assert(globalStyles.includes('grid-template-columns:52px 52px 52px') && globalStyles.includes('.rules-dialog ol{gap:11px;font-size:14px'), 'D-pad ponsel dan teks aturan diperbesar agar mudah dibaca');
...
assert(prototypeSource.includes("const MUSIC_MUTED_STORAGE_KEY = 'bentengan:music-muted'") && prototypeSource.includes('window.localStorage.setItem(MUSIC_MUTED_STORAGE_KEY') && prototypeSource.includes('if (!audioUnlocked || musicMuted) return'), 'opsi mute musik latar tersimpan tanpa mematikan sound effect');
assert(prototypeSource.includes("new Audio(uiAudioAsset('ingame-ambience.mp3'))") && prototypeSource.includes('}, [audioUnlocked, mode]);'), 'suara ambience pertandingan tetap aktif ketika musik latar dimatikan');
...
assert(fieldBaseline.version === 8, 'golden baseline field v8 mencakup sumber dan runtime Map 1–4 final');
for (const [filename, hash] of Object.entries(fieldBaseline.mapSources ?? {})) assert(hash === await sha256(`Assets/map/${filename}`), `${filename}: sumber map cocok golden baseline`);
...
for (const id of ids) {
  ...
  if (id === 'jago') {
    assert(animation.directions.south.boost.length === 4, 'jago: boost south memakai 4 frame');
    assert(animation.directions.east.boost.length === 6, 'jago: boost east memakai 6 frame asli');
    assert(animation.directions.west.mirror === 'east', 'jago: arah kiri memirror strip kanan');
  } else {
