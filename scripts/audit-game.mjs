...
const kampungOpenCount = (kampungOpenSection?.[1].match(/\{ x:/g) ?? []).length;
assert(kampungOpenCount >= 26 && fieldObstacleCounts.pasar >= 26 && fieldObstacleCounts.taman >= 26 && fieldObstacleCounts.kanal >= 26, `empat arena memiliki kepadatan halangan bermakna (${kampungOpenCount}/${fieldObstacleCounts.pasar}/${fieldObstacleCounts.taman}/${fieldObstacleCounts.kanal})`);
...
assert(prototypeSource.includes("difficulty: 'easy'") && prototypeSource.includes("difficulty: 'normal'") && (prototypeSource.match(/difficulty: 'hard'/g) ?? []).length === 2, 'tingkat kesulitan arena tersusun Easy, Normal, Hard, Hard');
