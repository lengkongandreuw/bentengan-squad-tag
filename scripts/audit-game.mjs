...
assert(fieldManifest.version === 6 && Object.keys(fieldManifest.objects.assets).length === 75, 'manifest field v6 memuat 75 objek statis serta terrain Map 1, Map 2, dan Map 3');
...
const fieldRuntimeBytes = (await stat(fieldObjectsPath)).size + (await stat(fieldAnimatedPath)).size + (await stat(fieldGroundsPath)).size + (await stat(kampungMapPath)).size + (await stat(pasarMapPath)).size + (await stat(tamanMapPath)).size;
const fieldDecodedBytes = (fieldObjects.width ?? 0) * (fieldObjects.height ?? 0) * 4 + (fieldAnimated.width ?? 0) * (fieldAnimated.height ?? 0) * 4 + (fieldGrounds.width ?? 0) * (fieldGrounds.height ?? 0) * 4 + (kampungMap.width ?? 0) * (kampungMap.height ?? 0) * 4 + (pasarMap.width ?? 0) * (pasarMap.height ?? 0) * 4 + (tamanMap.width ?? 0) * (tamanMap.height ?? 0) * 4;
assert(fieldObjects.width === 2048 && fieldObjects.height === 2176 && fieldObjects.hasAlpha, 'atlas objek statis v6 2048×2176 transparan dan terpotong rapat');
...
assert(tamanMap.width === 1923 && tamanMap.height === 1082 && !tamanMap.hasAlpha, 'terrain Map 3 memiliki dimensi gameplay 15% lebih luas tanpa kanal alpha');
assert(fieldRuntimeBytes <= 2800 * 1024, `enam aset field ${(fieldRuntimeBytes / 1024).toFixed(0)} KiB berada dalam budget 2.800 KiB`);
assert(fieldDecodedBytes <= 48 * 1024 * 1024, `memori decode field ${(fieldDecodedBytes / 1024 / 1024).toFixed(1)} MiB berada dalam budget 48 MiB`);
...
assert(prototypeSource.includes("getFieldImage('objects.webp')") && prototypeSource.includes("getFieldImage('animated.webp')") && prototypeSource.includes("getFieldImage('grounds.webp')") && prototypeSource.includes("background: 'kampung-map.webp'") && prototypeSource.includes("background: 'pasar-map.webp'") && prototypeSource.includes("background: 'taman-map.webp'"), 'dekorasi memakai tiga atlas dan tiga map revisi memakai terrain komposit khusus');
...
assert(prototypeSource.includes('const STATIC_MAP_SCALE = 0.5') && prototypeSource.includes('staticLayer.width = Math.round(worldWidth * STATIC_MAP_SCALE)'), 'cache visual setiap arena diraster setengah resolusi agar hemat memori');
assert((prototypeSource.match(/objectScale: (?:0\.9|MAP_OBJECT_SCALE)/g) ?? []).length >= 3 && prototypeSource.includes('(x + w / 2) * scaleX - mapW(w) / 2') && prototypeSource.includes('item.hidden ||') && prototypeSource.includes('item.underlay ||'), 'Map 1, Map 2, dan Map 3 diperluas 15%, objek diperkecil 10% dari titik tengah, dan collider bantu atau margin tidak digambar ulang');
...
assert(prototypeSource.includes('const GUIDE_FIELD_CONFIGS: FieldConfig[]') && prototypeSource.includes("background: 'taman-map.webp'") && prototypeSource.includes("'canalBridgeDiag'"), 'empat arena v3 memakai konfigurasi panduan dan objek modular baru');
...
assert(fieldBaseline.version === 6, 'golden baseline field v6 mencakup sumber Map 1, Map 2, dan Map 3');
