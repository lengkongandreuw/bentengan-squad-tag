...
const runtimeNames = ['animated.webp', 'grounds.webp', 'kampung-map.webp', 'pasar-map.webp', 'taman-map.webp', 'kanal-map.webp', 'kanal-water-mask.png', 'manifest.json', 'objects.webp'];
...
await writeFile(path.join(root, 'config', 'field-baseline.json'), `${JSON.stringify({ version: 8, sources, mapSources, runtime }, null, 2)}\n`);
console.log(`Field baseline v8 updated: ${sourceNames.length} object sources + ${mapSourceNames.length} map cuts + ${runtimeNames.length} runtime files.`);
